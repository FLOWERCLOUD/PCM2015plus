...
emit writeSignal(QString("recordS: ")+ QString("%1").arg(recordS)+QString("  recordE: ")+QString("%1").arg(recordE)+QString("  vtxBSize: ")+QString("%1").arg(vtxBSize) ,3);  //释放写信号,写到主窗口的输出框中
...
std::cout<<"thread waking"<<std::endl;
Logger<<"边的起点为"<<curEdgeStart<<"终点为"<<curEdgeEnd<<endl;	
emit writeSignal(QString("begin: edge start: ")+ QString("%1").arg(curEdgeStart)+QString("  edge end: ")+QString("%1").arg(curEdgeEnd) ,3);  //释放写信号,写到主窗口的输出框中
QMutex mutex;
mutex.lock();
mWaitcond.wait(&mutex);
mutex.unlock();
...
emit writeSignal(QString("recordS: ")+ QString("%1").arg(strCorPsSzie)+QString("  recordE: ")+QString("%1").arg(endCorPsSize) +QString("  vtxBSize: ")+QString("%1").arg(vtxBSize),3);  //释放写信号,写到主窗口的输出框中
