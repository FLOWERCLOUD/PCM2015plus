...
namespace nanoflann {
    ...
    struct KDTreeAdaptor {
		...
		inline size_t radiusSearch(const num_t *query_point, num_t radius, std::vector<std::pair<IndexType, num_t> >& indicesDist)
		{
			return index->radiusSearch(query_point, radius, indicesDist, nanoflann::SearchParams());
		}
...
namespace SICP {
    ...
    inline void shrink(Eigen::Matrix3Xf& Q, float mu, float p) {
        float Ba = std::pow((float)(2.0/mu)*(1.0-p), (float)1.0/(2.0-p));
